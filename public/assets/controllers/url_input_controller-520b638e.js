...
export default class extends Controller {
  ...
  handleSubmit(event) {
    // Don't prevent default for now - let the form submit naturally after animation
    const url = this.inputTarget.value
    if (this.isValidUrl(url)) {
      event.preventDefault() // Only prevent if we're showing animation
      ...
      // 4초 후에 실제 폼 제출
      setTimeout(() => {
        // Remove the event handler to avoid infinite loop
